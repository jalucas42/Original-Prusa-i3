...
module y_rod_holes(open_end=0) {
    ...
    translate([(open_end*-5)+5,-50/2,y_rod_to_rail+y_rod_diam*0.15]) cube([27+2,50,10]);
    
    // Ziptie cutout
    translate([27-5-thinwall,0,y_rod_to_rail]) rotate([0,90,0]) rotate_extrude(angle=360) translate([y_rod_diam/2+2,0]) square([2,5]);
    ...
    for (i = [0:1]) {
        // Screw hole
        mirror([0,i,0]) translate([27/2,17,-1]) rotate([0,0,0]) cylinder(r=1.7, h=100);
        
        //// Screw head cutout
        //mirror([0,i,0]) translate([27/2,17,4]) rotate([0,0,0]) poly_cylinder(r=6.5/2, h=100);
        
        // Captive nut hole
        mirror([0,i,0]) translate([27/2,17,4]) rotate([0,0,0]) cylinder(r=6.3/2, h=100, $fn=6);
    }
